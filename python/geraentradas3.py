...
def write_byte(val):
    ...
    outputBuffer.append(val & 0xFF)
    outputPtr += 1


def write_bytes(data):
    global outputBuffer, outputPtr
    outputBuffer.extend(data)
    outputPtr += len(data)
...
def DCT(block, stride, offset=0):
    ...
    add07 = block[b(0)] + block[b(7)]
    sub07 = block[b(0)] - block[b(7)]
    add16 = block[b(1)] + block[b(6)]
    sub16 = block[b(1)] - block[b(6)]
    add25 = block[b(2)] + block[b(5)]
    sub25 = block[b(2)] - block[b(5)]
    add34 = block[b(3)] + block[b(4)]
    sub34 = block[b(3)] - block[b(4)]
    add0347 = add07 + add34
    sub07_34 = add07 - add34
    add1256 = add16 + add25
    sub16_25 = add16 - add25
...
def writeJpeg(pixels, width, height, filename):
    ...
    for i in range(64):
        ...
        chr_ = int((DefaultQuantChrominance[ZigZagInv[i]] * qFactor + 50) / 100)
        quantLuminance[i] = clamp(lum, 1, 255)
        quantChrominance[i] = clamp(chr_, 1, 255)
    ...
    write_byte(height >> 8)
    write_byte(height & 0xFF)
    write_byte(width >> 8)
    write_byte(width & 0xFF)
    ...
    with open(filename, "wb") as f:
        f.write(outputBuffer)  # salva o arquivo imagem
...
for x in range(qtdimagens):
    ...
    if x < qtdimagens / 2:
        cx = 120 + random.randint(0, 160)
        cy = 120 + random.randint(0, 160)
        ...
        cx = 10 + random.randint(0, 180)
        cy = 10 + random.randint(0, 180)
        for y in range(height):
            for px in range(width):
    ...
    filename = f"imagens/amostra{x:04d}.jpg"
    writeJpeg(pixels, width, height, filename)
